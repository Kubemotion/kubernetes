	...
	for _, test := range enoughPodsTests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !fits && !reflect.DeepEqual(reasons, test.reasons) {
				t.Errorf("%s: unexpected failure reasons: %v, want: %v", test.name, reasons, test.reasons)
			}
			if fits != test.fits {
...
func TestPodFitsHost(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if fits != test.fits {
				t.Errorf("%s: unexpected difference: expected: %v got %v", test.name, test.fits, fits)
			}
		})
	}
}
...
func TestPodFitsHostPorts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.fits != fits {
				t.Errorf("%s: expected %v, saw %v", test.name, test.fits, fits)
			}
		})
	}
}
...
func TestGCEDiskConflicts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.isOk && !ok {
				t.Errorf("%s: expected ok, got none.  %v %s %s", test.name, test.pod, test.nodeInfo, test.name)
			}
			if !test.isOk && ok {
				t.Errorf("%s: expected no ok, got one.  %v %s %s", test.name, test.pod, test.nodeInfo, test.name)
			}
		})
	}
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !fits && !reflect.DeepEqual(reasons, test.expectFailureReasons) {
				t.Errorf("%s: unexpected failure reasons: %v, want: %v", test.name, reasons, test.expectFailureReasons)
			}
			if fits != test.fits {
				t.Errorf("%s: expected %v got %v", test.name, test.fits, fits)
			}
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for indexNode, node := range test.nodes {
				...
				if !fits && !reflect.DeepEqual(reasons, test.nodesExpectAffinityFailureReasons[indexNode]) {
					t.Errorf("index: %d test: %s unexpected failure reasons: %v expect: %v", indexTest, test.name, reasons, test.nodesExpectAffinityFailureReasons[indexNode])
				}
				...
				if affinity != nil && affinity.NodeAffinity != nil {
					...
					if !fits2 && !reflect.DeepEqual(reasons, selectorExpectedFailureReasons) {
						t.Errorf("%s: unexpected failure reasons: %v, want: %v", test.name, reasons, selectorExpectedFailureReasons)
					}
					fits = fits && fits2
				}

				if fits != test.fits[node.Name] {
					t.Errorf("%s: expected %v for %s got %v", test.name, test.fits[node.Name], node.Name, fits)
				}
			}
		})
	}
...
func TestPodToleratesTaints(t *testing.T) {
	...
	for _, test := range podTolerateTaintsTests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !fits && !reflect.DeepEqual(reasons, expectedFailureReasons) {
				t.Errorf("%s, unexpected failure reason: %v, want: %v", test.name, reasons, expectedFailureReasons)
			}
			if fits != test.fits {
				t.Errorf("%s, expected: %v got %v", test.name, test.fits, fits)
			}
